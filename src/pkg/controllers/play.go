...
func PrintDeck(client *Client, d []cards.Card) {
	s := ""
	for _, card := range d {
		s += card.ToString() + " "
	}
...
func ShowCards(client *Client, sp []Player) {
	realPlayers := 0
	for _, p := range sp {
		if !p.IsBot {
			realPlayers++
...
func Play(client *Client, r *Round, playerIndex int, bigBlind int, totalStacks float64, coeff Coefficients, botMode bool) {
	...
	if !p.IsAllIn && !p.HasFolded {
		if !p.IsBot {
			...
			if p.InitialStack > (*r).MaxBet {
				client.send <- []byte(fmt.Sprintf("Raise (r)"))
			}
			client.send <- []byte(fmt.Sprintf("Fold (f)"))
			validInput := false
			for !validInput {
				validInput = true
				...
				if !validInput {
					client.send <- []byte(fmt.Sprintf("Your choice has not been understood"))
				}
			}
		} else {
...
func (g *Game) NewRound(client *Client, botMode bool) {
	d := cards.NewDeck()
	d = cards.Shuffle(d)
	players := []Player{}
	biggestStack := 0
...
func (g Game) BetBlinds(client *Client, botMode bool) {
...
func (g *Game) EndRound(client *Client, botMode bool) {
...
var bots = []string{"Oscar", "Julia", "Abel", "Anne", "Pamela"}
...
func (r *Round) Bet(client *Client, playerIndex int, amount int, botMode bool) {
	bet := cards.MinInt(amount, (*r).LimitBet-(*r).Players[playerIndex].RoundBet)
	if (*r).Players[playerIndex].InitialStack-(*r).Players[playerIndex].RoundBet <= bet {
		...
		(*r).MaxBet = cards.MaxInt((*r).Players[playerIndex].InitialStack, (*r).MaxBet)
