...
func StraightOdds(commonCards []Card, hs HandScore, hand []Card) float64 {
	...
	for _, card := range commonCards {
		if !Contains(simpleCards, card.Rank) {
			simpleCards = append(simpleCards, card.Rank)
		}
	}
	simpleCards = OrderSliceOfInt(simpleCards)
	cardToDefeat := 0
	if hs.Score == 400 {
...
func HighCardOdds(cc []Card, hand []Card, remainingCardsNb float64) float64 {
	...
	allCardsKnown = OrderSliceOfIntDesc(allCardsKnown)

	if index := IndexOf(allCardsKnown, MaxInt(hand[0].Rank, hand[1].Rank)); index < 3 {
		highCard1ToDefeat = allCardsKnown[index]
	} else {
		highCard1ToDefeat = allCardsKnown[3]
	}
	if index := IndexOf(allCardsKnown, MinInt(hand[0].Rank, hand[1].Rank)); index < 4 {
	...
	if MaxInt(hand[0].Rank, hand[1].Rank) == highCard1ToDefeat {
